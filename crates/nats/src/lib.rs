...
impl ChatLock {
    // Pre-existing chat code: the lock carries the whole requested turn.
...
fn duration_from_env(name: &str, default_seconds: u64) -> Duration {
    Duration::from_secs(env_u64(name, default_seconds))
}

fn env_u64(name: &str, default_seconds: u64) -> u64 {
    std::env::var(name)
        .ok()
