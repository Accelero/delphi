...
async fn scheduler_persists_and_advances_cursor() {
    let storage = Arc::new(
        SurrealStorage::in_memory("scheduler_test", "main")
            .await
            .expect("connect"),
    );
    ...
    let handle = run_scheduler(sink, filter, trait_storage.clone(), registry);
    ...
    let cursor = trait_storage.get_source_cursor("fake").await.unwrap();
