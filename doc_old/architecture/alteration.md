# Chat Workflow Alterations

Status: walkthrough notes. Apply these to the architecture/gap analysis after
the workflow review is complete.
...
- `text` must be bounded by the chat message limit while we do not have a
  separate content/object store.
...
- Remove `worker_id` from the `CHAT_LOCKS` schema if it is no longer needed
  for ownership validation/debugging. The lock owner is the active holder of
  the conversation claim, not the stop routing address.
