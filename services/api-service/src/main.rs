...
use delphi_document_adapters::{config, DocumentInfra, SystemClock, UlidGen};
use delphi_document_app::DocumentService;
use delphi_nats::{ChatBus, ChatBusError, ChatLock, NatsChatBus, NatsChatBusOptions};
use delphi_storage::{ChatRepository, PgRepository, StorageError};
use documents::DocumentApiState;
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Owns the migration runner, so the schema is current before either the
    // chat repository or the document read model touches a table.
    // api-service authors the topology: it is the writer that creates upload
    // records, so the bucket's lifetime belongs with it. Nothing else declares.
    let topology = config::TopologyConfig::from_env()?;
    let infra = DocumentInfra::connect_as_author(
        &config.database_url,
    ...
    .await?;
    let api_config = config::ApiConfig::from_env()?;

    let documents = DocumentApiState {
        auth: auth.clone(),
        service: Arc::new(DocumentService::new(
            infra.events.clone(),
            infra.blobs.clone(),
            infra.uploads.clone(),
            infra.read_model.clone(),
            infra.work_queue.clone(),
            Arc::new(SystemClock),
            Arc::new(UlidGen),
            config::upload_policy(&api_config),
        )),
    };
    ...
    let app = Router::new()
        ...
        .with_state(state)
        // The document routes carry their own state and their own error
        // mapping; they share only the auth verifier.
        .merge(documents::routes(documents))
