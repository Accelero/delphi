...
Application code does not import SurrealDB types directly. It depends on
the `Storage` trait (`backend/src/storage/mod.rs`); the SurrealDB
implementation is one concrete backend behind that trait. A future
Postgres + Qdrant backend would be added by writing a second `impl
Storage` and registering it in `storage_from_env`, without touching
callers.
...
- **Backend-specific power features.** SurrealDB's graph traversals,
  live queries, and custom functions are accessible only through the
  `SurrealStorage` concrete type. Higher-level subsystems that want them
  can import `SurrealStorage` directly and accept the coupling. The
  trait surface stays small and portable.
- **Sessions, auth bootstrap upserts, and other "system" tables** stay
  off the `Storage` trait. They go straight against `Surreal<Any>` from
  `auth/bootstrap.rs` etc. — there's no value in abstracting them
  because no one else needs to swap them out.
...
- SurrealDB record IDs (e.g. `document:abc…`) are an internal detail.
  External code passes them around as opaque strings.
...
- **Graph edges.** The schema does not yet define relations like
  ...
  designed.
- **Tenant isolation enforcement.** Application code passes `tenant_id`
  on every write; we have not yet enabled SurrealDB's record-level
  access rules. Planned for the multi-tenant SaaS deployment.
