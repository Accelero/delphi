...
**Sister docs:** [`ingestion-v2.md`](./ingestion-v2.md),
[`ingestion-roadmap.md`](./ingestion-roadmap.md) (§0), the metadata
validator in [`metadata-extractor.md`](./metadata-extractor.md).
...
It supersedes the single-function `validation/object.rs` that exists
today (byte-authoritative sniff, wired in always-accept mode at
`completion.rs:77`).
