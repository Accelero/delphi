...
plan for what is not yet built lives in
[`ingestion-roadmap.md`](./ingestion-roadmap.md).

> Supersedes the design notes in [`ingestion-v2.md`](./ingestion-v2.md)
> and the build plan in [`upload-ui.md`](./upload-ui.md); those are kept
> as historical design/review artefacts. This document describes what is
...
2. **Extract text** (`ingestion/text_extract.rs`) — one **bounded**
   ...
   non-fatal. *(This is a deliberate, capped exception to "bytes never
   traverse the backend" — documented in [`../SECURITY.md`](../SECURITY.md).)*
