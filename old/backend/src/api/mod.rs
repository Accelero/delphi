...
pub fn build_router(
    ...
    // additionally prepend `dev_inject_middleware`, which mints a fixed
    // bearer JWT — making the dev path a strict subset of the production
    // path (same extractor, same upsert, same AuthContext).
