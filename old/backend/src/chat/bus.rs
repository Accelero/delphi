...
//!
//! Phase 1 ships one implementation, [`super::inprocess::InProcessBus`]
//! (single instance, in-memory). The NATS migration adds a second impl
//! behind this same trait; the wire contract (opaque [`Cursor`] on every
//! frame + `resync`) is what makes that swap invisible to the worker,
//! handlers, and client. See
...
/// shape (in-memory now, NATS later); selected once at startup and held
...
pub trait TurnBus: Send + Sync {
    ...
    /// subscribed. Clients handle it idempotently. Maps to a NATS publish
    /// in the scale-out backend — same seam.
...
///
/// Concrete and in-process for Phase 1 (the seam that survives the NATS
