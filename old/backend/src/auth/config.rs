...
//! Two modes:
//!
//! - [`AuthMode::Header`]: production path. Despite the legacy enum name,
//!   identity arrives as `Authorization: Bearer <jwt>` forwarded by the
//!   BFF. The backend validates the JWT and passes it to SurrealDB
//!   `app_session` access.
...
/// Settings the backend needs in production mode. The name is legacy from
/// the pre-JWT header projection era; the remaining setting is only the
/// default tenant fallback.
#[derive(Debug, Clone)]
pub struct HeaderConfig {
    /// Slug of the tenant a request lands in when the JWT has no
    /// `tenant_id` claim.
