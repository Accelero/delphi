...
- **Tier 1 (`docker-compose.yml`).** SurrealDB + backend (with the
  `dev-auth` cargo feature) + Vite frontend. The dev injector mints a
  JWT with the same claim shape as production; downstream auth pipeline
  runs unchanged.
  Fast smoke tests. URL: `http://localhost:5173`.
- **Tier 2 (`docker-compose.full.yml`).** Traefik + Keycloak (OIDC IdP) +
...
- **Auth-boundary tests** for `JwtClaimsExtractor` and validators:
  malformed, expired, wrong-signature, and missing-claim JWTs.
...
- **Equivalence test** between dev-mode JWT minting and the production
  JWT extractor — already in place (`auth/dev.rs::tests`).
...
- Additional auth-boundary fuzz/property coverage.
