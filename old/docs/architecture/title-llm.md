...
([`metadata-extractor.md`](./metadata-extractor.md)) is another non-chat
LLM consumer that can point at a sidecar instead of burning the chat
model. If more utility consumers land, promote `title_llm` /
`metadata_llm` to a small **utility-LLM registry** (a keyed map on
`AppState`) rather than one bespoke field per feature. Also note: if we ever standardize on
